...
import Environment.Influence_map.Influence_map as Influ
...
def _build_model(input_data, output_data, size=(0, 0)) -> Network:
    """
    ...
    :return: Model
    """
    return Network(input_data, output_data, size[0], size[1]).to(device)
...
def _load_data(sampling_size, mode=False):
    if mode:
        Data = random.sample(Influ_data, sampling_size)
    else:
   ...
   """
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            return main_model(state).max(1)[1].view(1, 1), eps_threshold
    else:
        return torch.tensor([[random.randrange(0, 5)]], device=device, dtype=torch.long), eps_threshold
...
def _optimize_model():
    ...
    batch = _load_data(parameter['experience'], False)
...
def train(max_steps=100000, train_num=100000) -> None:
    ...
    if parameter['epsilon_decay'][0] == 'Auto':
        parameter['epsilon_decay'][1] = int((train_num * 100) / (parameter['epsilon_start'] - parameter['epsilon_end']))  # 트레이닝 횟수 증가에 따른 decay 변화 자동화

    env = Env.make_env(parameter['game_set'])
    steps = 0
    ...
    for i in tqdm(range(1, train_num + 1)):
        ...
        while True:
            action, eps_threshold = _action_predict(state, steps)  # 행동 계산
            obs, reward, done, info = env.step(action)
            if reward != 0 and reward <= 50:
                reward = 1
            elif reward > 50:
                reward = -2
            ...
            done = done or info['ale.lives'] != 3 or ingame_steps >= max_steps
            if done:
                next_state = None
                reward = -2
                total_reward = 0
            ...
            data = _save_data(state, next_state, action, torch_reward)
...
def test():
    game = Env.make_env(parameter['game_set'])
